...
the legacy MPO APIs use the term `campaign id` to refer to what this api calls `adSetid`is the identifier your integration knows as `campaign id.` The `campaign id` dimension and filter are available for MPO Pro only Multi-seller and Single-Seller integrations at ad set level.
...
Even if `startDate` and `endDate` fall within the standard 1-year lookback limit, data may not be available for the full period at launch. During the initial rollout, available data starts from **\[DATE TO CONFIRM BEFORE RELEASE\]** and grows daily; requests with a `startDate` before this point will be rejected with an `invalid-query` error. Full 1-year depth is expected to be reached progressively after go-live.
