...
Usually if advertiser hasn't given a consent to your app you will get an error in 4xx range. Part 1 of the tutorial covers 4xx error series in more detail here: [How to handle API errors](doc:api-error-types). In order to perform a quick check whether an advertiser has given consent to the app you can query [/advertisers/me endpoint](https://developers.criteo.com/marketing-solutions/reference-link/apiportfolioget) and see if it returns advertiser ID that you expect.

**Implementation errors**

If the request is not formed according to the \[specification] (ref:apiv1oauth2tokenpost) or an invalid request header is provided, the server will respond with an error in the 400 code series. Part 1 of the tutorial covers 4xx error series in more detail here: [How to handle API errors](doc:api-error-types).
...
Starting from [How to handle API errors - Part 1](doc:api-error-types) to now, we have focused on common issues that can arise with the API.
...
Aligning the timezones and the date ranges in the API request body and the platform UI (whether Criteo Commerce Growth or a third-party platform) will resolve the issue most of the time. Check [this document](doc:campaign-statistics#timezones) for the list of supported timezones.

**Attribution scope**

Please view [this document](doc:campaign-statistics#metrics) for the list of supported metrics in the Criteo API.
