...
| `mediaChannel` | array[string] | No | Filters to specific media channels (currently only `"AI Platform"` exists). Omit for no filter. |
| `platform` | array[string] | No | Filters to specific platforms (currently only `"OpenAI"` exists). Omit for no filter. |
| `currency` | string | No — defaults to `EUR` | ISO 4217 code. Converts `Spend`/`Cpc`/`Cpm` into this currency. An unrecognized code doesn't error — it's a soft no-op (the value is returned unconverted, labeled with whatever code was sent). |
...
| `Day` | Calendar day the row belongs to, formatted `YYYY-MM-DD`. Always a UTC calendar day — there's no finer time-of-day grain in the underlying data to convert. |
...
| `Spend` | — | Sum of spend, returned as `{ "value": <number>, "currency": "<requested code>" }`, converted into the currency requested via `currency` (defaults EUR) |
| `Ctr` | — | Clicks / Impressions |
| `Cpc` | — | Spend / Clicks, same currency conversion as `Spend` |
| `Cpm` | — | Spend / Impressions × 1000, same currency conversion as `Spend` |
...
RequestBody body = RequestBody.create(mediaType, "{\"data\":{\"type\":\"AdLevelReportQuery\",\"attributes\":{\"advertiserIds\":[\"122530\"],\"adsetIds\":[\"708590\"],\"startDate\":\"2026-07-14\",\"endDate\":\"2026-07-22\",\"format\":\"json\",\"dimensions\":[\"AdGroupName\",\"ProductId\"],\"metrics\":[\"Impressions\",\"Clicks\",\"Spend\",\"Ctr\"],\"currency\":\"USD\"}}}");
...
$request->setBody('{"data":{"type":"AdLevelReportQuery","attributes":{"advertiserIds":["122530"],"adsetIds":["708590"],"startDate":"2026-07-14","endDate":"2026-07-22","format":"json","dimensions":["AdGroupName","ProductId"],"metrics":["Impressions","Clicks","Spend","Ctr"],"currency":"USD"}}}');
try {
