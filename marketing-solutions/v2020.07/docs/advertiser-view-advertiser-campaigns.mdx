---
title: "[Advertiser] View Advertiser Campaigns"
---

1. Guides for Marketing Solutions API
2. [Campaigns](/marketing-solutions/v2020.07/docs/campaign)

## **Retrieving All Campaigns at the Advertiser Level**

For the Advertiser API, all campaigns for the specified advertiser will be returned.

This includes campaigns that are not currently running as well as those that have been archived.  

![](/images/marketing-solutions/v2020.07/docs/8a6bfd9-get.png)

```
https://api.criteo.com/legacy/marketing/v1/advertisers/{advertiserId}/campaigns
```

<CodeGroup>
  ```bash bash
  curl --request GET \
    --url https://api.criteo.com/legacy/marketing/v1/advertisers/1234/campaigns \
    --header 'Accept: application/json'
  ```
</CodeGroup>

 

The following is an example of what's returned:

<CodeGroup>
  ```bash bash
  [
    {
      "campaignType": "LowerFunnel",
      "advertiserName": "Advertiser US",
      "categories": [],
      "budgetId": 99999,
      "campaignId": 1234,
      "campaignName": "Web Conversion - Retargeting",
      "advertiserId": 1,
      "campaignStatus": "Running",
      "campaignBid": {
        "bidValue": 1.25,
        "bidCurrency": "USD",
        "bidType": "CPC"
      }
    },
    {
      "campaignType": "MidFunnel",
      "advertiserName": "Advertiser US",
      "categories": [],
      "budgetId": 99998,
      "campaignId": 1235,
      "campaignName": "Web Conversion - Similar Audience",
      "advertiserId": 1,
      "campaignStatus": "Running",
      "campaignBid": {
        "bidValue": .90,
        "bidCurrency": "USD",
        "bidType": "CPC"
      }
    },
  ```
</CodeGroup>

 

This same information can be pulled utilizing the **Campaign API** with the following query:

![](/images/marketing-solutions/v2020.07/docs/8a6bfd9-get.png)

```
https://api.criteo.com/legacy/marketing/v1/campaigns?advertiserIds={advertiserIds}
```

 

## Filtering Campaigns

If you would like to **filter** certain campaigns using the Campaign API, such as those that are running or a specific campaign, the process to do this is documented in the [Campaign](doc:campaign) section.

***

[Campaigns](/marketing-solutions/v2020.07/docs/campaign)

[\[Campaigns\] Get Existing Campaigns](/marketing-solutions/v2020.07/docs/get-existing-campaigns)
