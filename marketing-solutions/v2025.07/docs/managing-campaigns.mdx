...
**`suspendedSince`** Date and time since we stopped delivering ads for this Seller in this Campaign.

**`suspensionReasons`** List of reasons why the campaign is suspended (possible values are: `NoMoreBudget`, `ManuallyStopped`, `NoBudgetDefined`, `NoCpcDefined`, `RemovedFromCatalog`)

Every Seller-Campaign is identified by:

**`sellerId`** `SellerId` of the seller associated to this Seller-Campaign

**`campaignId`** `CampaignId` of the MPO-campaign associated to this Seller-Campaign
...
If no filters are set, it returns all accessible budgets except those with an `endDate` in the past. When multiple filters are applied, only budgets meeting all criteria are included.
...
* have a bid (see "[Setting the Bid for a given Seller-Campaign](doc:managing-campaigns#setting-the-bids-of-seller-campaigns)" section),
* have an active budget (see "[Managing Budgets](doc:managing-budgets)" section),
...
* set the bid of the Seller-Campaign to null (see "[Setting the Bid for a given Seller-Campaign](doc:managing-campaigns#setting-the-bids-of-seller-campaigns)" section),
* suspend the current budget (see "[Suspend a Budget](doc:managing-budgets#suspending-budgets)" section),
