...
**`offset`** 0 based index of the first coupon to return in the response. The default is 0.

**`limit`** The number of coupons to be returned. The default is 50.
...
A new coupon can be created for a specific advertiser by making a `POST` call to the coupons' endpoint. The request body should specify the name, ad set ID, description (optional) and the coupon attributes.

### Coupon Attributes

**`landingPageUrl `**&#x57;eb redirection of the landing page url.

**`startDate`** The date when the Coupon will be launched.

**`endDate`** The date when the coupon will stop being displayed. If the end date is not specified (i.e. null), the coupon will always be displayed.

**`format`** Format of the coupon. Can be `FullFrame `or `LogoZone`.

**`images`** List of slides images.

**`showEvery`** Show the coupon every N seconds. Can be between 1 and 10.

**`showDuration`** Show coupon for a duration of N seconds. Can be between 1 and 5.

**`rotationsNumber`** Number of rotations for the coupons. The number can be between 1 and 10.
...
**`invalid-html-tag`** One of the HTML tags is not supported. Please check the list of supported ad servers [here](https://help.criteo.com/kb/guide/en/third-party-ads-QUSq5Astwy/Steps/775787,817360)
...
[Marketplace Performance Outcomes](/marketing-solutions/v2025.10/docs/marketplace-performance-outcomes)
