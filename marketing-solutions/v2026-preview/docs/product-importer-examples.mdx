...
**Action** Given the error's detail, fix your request and then re submit.
...
**Request** Assuming the user post this payload along with an invalid bearer token
...
**Action** Request a new token from the authentication endpoint and then retry.

### **`403`**

**Request** Assuming the current authenticated user doesn't have the permission to manage merchant (aka partner) 123
...
**Action** Check the credentials you used to authenticate, check the merchantId you used, fix and the retry.
...
**Response** No body. **Action** Reduce size of text field if any is over the field capacity or reduce the size of the batch, and then retry.
...
**Action** Wait for the time provided before retrying using an exponential backoff algorithm. Consider checking the number of products you're trying to send. The default limit is set to 3000 products / s per merchant.
...
**Action** Retry using a exponential backoff algorithm.
...
**Action** The batch has been accepted, its treatment hasn't started yet.
...
**Action** The batch has been fully validated without any error.
...
**Action** Because Criteo apply some transformation to adapt the incoming format to Criteo product definition, it may happen that Criteo fails to transform a product. Such issues can be fixed by either changing the product definition or the transformation procedure.
...
**Action** Check the credentials you used to access this operation is authorized to access this partner's operation.
...
**Action** This operation hasn't been found. This could happen in case of a severe unavailability. Criteo recommends to resubmit the entire batch.

### **`413`**

**Request** Any corrupted token such as the payload is really too heavy. **Response** No body. **Action** Fix.

### **`429`**

**Request** Assuming you're sending too many requests and hit your limit. Criteo throttles your requests.
...
**Action** Wait for the time provided before retrying using an exponential backoff algorithm. As the average time to get a report of 1000 products should be less than 3 seconds, Criteo recommends to start polling after 3 seconds and then retry using the exponential backoff algorithm.
...
[Product Importer Statistics](/marketing-solutions/v2026-preview/docs/product-importer-statistics)
