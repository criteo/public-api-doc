---
title: Ad Set
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Performance_Media/v2026-preview/docs/Preview Endpoints/campaigns/ad-set.md
---
...
**`objective`**  
The outcome the Ad Set is intended to optimize.

Supported values are `customAction`, `clicks`, `conversions`, `displays`, `appPromotion`, `revenue`, `storeConversions`, `reach`, `visits`, and `videoViews`.

An objective is not necessarily supported for every media type and cost controller. The API validates `objective`, `mediaType`, and `bidding.costController` together. See [Cost-controller compatibility](#cost-controller-compatibility) before creating an Ad Set.

`videoViews` is supported only when `mediaType` is `Video`. `revenue` is not supported when `mediaType` is `Video` and `bidding.costController` is `dailyBudget`.
...
**`campaignId`**  
Id of the marketing campaign associated to the ad set. See [Search for Campaigns](/marketing-solutions/v2026-preview/docs/campaign#searching-campaigns) for how to search for campaigns.
...
Can be `on` or `off`. `off` by default. Set by the advertiser, this represents the intent to deliver ads. See [Start and Stop Ad Sets](/marketing-solutions/v2026-preview/docs/ad-set#start-an-ad-set) for how to change this value.
...
How spend is controlled and optimized. Can be `COS` (Cost of Sale), `MaxCPC` (Cost per Click), `CPI` (Cost per Install), `CPM` (Cost per Mille), `CPO` (Cost per Order), `CPSV` (Cost per Site Visit), `CPV` (Cost per View), `targetCPM` or `dailyBudget`.

### Cost-controller compatibility

A cost-controller value is not valid with every combination of `mediaType` and `objective`. The API uses these three fields together to select the Ad Set's optimization model.

For budget-controlled performance Ad Sets, the following combinations are supported:

| `mediaType` | `objective` | `bidding.costController` | Optimization behavior |
|---|---|---|---|
| `Display` | `visits` | `dailyBudget` | Maximize qualified visits while delivering toward the budget. |
| `Display` | `conversions` | `dailyBudget` | Maximize attributed conversions while delivering toward the budget. |
| `Display` | `revenue` | `dailyBudget` | Maximize attributed revenue or order value while delivering toward the budget. |
| `Video` | `visits` | `dailyBudget` | Maximize attributed post-view visits while delivering toward the budget. |
| `Video` | `conversions` | `dailyBudget` | Maximize attributed post-view conversions while delivering toward the budget. |

<Warning>
  **Video revenue optimization is not supported**

  Do not combine `mediaType: video`, `objective: revenue`, and `bidding.costController: dailyBudget`. Criteo does not currently provide a budget based video optimization model for attributed revenue or order value. Use `objective: conversions` to optimize a video Ad Set for post-view conversions.
</Warning>

Unsupported combinations are rejected during Ad Set creation with the `campaign--ad-set-creation-check--invalid-bidding-configuration` validation error.
...
The period by which the maximum impression limit is calculated. Can be `hourly`, `daily`, or `lifetime`.  `advanced` is available for special Criteo-configured setups, but cannot be selected or set manually.
...
The cadence of budget reneweal. Can be `daily`, `weekly`,`monthly`, `lifetime`, or `undefined`. Currently read-only.
...
**`budget.pacingBehavior`**  
Controls how a standard capped budget is paced. Nullable. Possible values:
...
**`scheduledBudget`**  
An object for managing time-bounded budgets on an ad set. Use this field to create, update, or delete scheduled budgets. On read, the ad set returns a `scheduledBudgets` array containing all active and future budgets.

**`scheduledBudget.scheduledBudgetCreations[]`**  
Array of new scheduled budgets to create. Each entry requires:
- `startDate` (date-time, required) — Date and time when the scheduled budget becomes active.
- `budgetAmount` (number, optional) — Maximum budget amount in the advertiser's currency per renewal period. Required when `budgetStrategy` is `capped`; must be omitted when `uncapped`.
- `budgetDeliverySmoothing` (string, optional) — Pacing strategy: `accelerated` or `standard`.
- `budgetDeliveryWeek` (string, optional) — Seven-day delivery window, e.g. `mondayToSunday`.
- `budgetRenewal` (string, optional) — Renewal cadence: `daily`, `weekly`, `monthly`, `lifetime`, or `undefined`.
- `endDate` (date-time, optional) — Date and time when the scheduled budget expires.

**`scheduledBudget.scheduledBudgetUpdates[]`**  
Array of changes to existing scheduled budgets. Each entry requires `id` (string). Optional fields are the same as for creations.

**`scheduledBudget.scheduledBudgetDeletions[]`**  
Array of scheduled budgets to remove. Each entry requires `id` (string) — the identifier of the budget to delete.
...
The API will return an array of the ID and attributes of the new ad set.  
The ad set attributes include the name, data set ID, campaign ID, advertiser ID, destination environment, activation status, delivery status, bid strategy and cost controller.  
If `attributionConfiguration` is not filled, then a default value of `attributionMethod: CriteoAttribution`, will be set.
...
**`trackingCode`**  
Required. Click URL parameters. Query string syntax without "?" (eg. _utm_source=criteo&utm_medium=display&utm_campaign=mycampaign_)

<Warning>
  **Ad Set Audiences**

  Newly created Ad Sets will not be linked to any audience.  
  To create an audience, refer to the [Create an Audience](/marketing-solutions/v2026-preview/docs/audience#create-audiences) guide.  
  You can link existing audiences following the steps in the [Update Ad Set Audience](/marketing-solutions/v2026-preview/docs/ad-set#update-adset-audience) guide.
</Warning>
...
For example, the following call would update the `name`, `startDate`, `endDate`, `bidAmount` and frequency capping settings of an existing Ad Set. The bid amount value supports a maximum of 4 decimal digits.  
If more digits are provided, the amount will be rounded.
...
Use this endpoint if you want to link an Audience to an AdSet.  
Both need to exist beforehand.
...
In addition to [general API errors](/marketing-solutions/v2025.07/docs/api-error-types) , you may encounter validation errors when updating an Ad Set or attempting to start and stop delivery.
...
The Ad Set has invalid audience or audience with missing parameters. See [Create an Ad Set](/marketing-solutions/docs/ad-set#create-an-ad-set) for how to configure Ad Set Audience.

**`campaign--ad-set-creation-check--no-audience-config`**  
The Ad Set has no audience configured. See [Create an Ad Set](/marketing-solutions/docs/ad-set#create-an-ad-set) for how to configure Ad Set Audience.

**`campaign--ad-set-creation-check--multiple-audience-configs`**  
The Ad Set has multiple audiences configured. Only one audience can be configured per Ad Set. See [Create an Ad Set](/marketing-solutions/docs/ad-set#create-an-ad-set) for how to configure Ad Set Audience.
...
The combination of `objective`, `mediaType`, and `bidding.costController` is not supported.

For example, `mediaType: Video`, `objective: revenue`, and `bidding.costController: dailyBudget` is invalid because no budget-based video revenue optimization model is available. Review the [cost-controller compatibility](#cost-controller-compatibility) guidance and select a supported combination.
...
This Ad Set is not linked to an audience. Check the section [Update Ad Set Audience](/marketing-solutions/docs/ad-set#update-adset-audience).
