---
title: Campaign
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Performance_Media/v2026-preview/docs/Preview Endpoints/campaigns/campaign.md
---
...
**`scheduledSpendLimit`**  
An object for managing time-bounded spend limits on a campaign. Use this field to create, update, or delete scheduled spend limits. On read, the campaign returns a `scheduledSpendLimits` array containing all active and future limits.

**`scheduledSpendLimit.scheduledSpendLimitCreations[]`**  
Array of new scheduled spend limits to create. Each entry requires:
- `spendLimitType` (string, required) — `capped` or `uncapped`.
- `startDate` (date, required) — Advertiser-local calendar date when the limit becomes active, e.g. `"2026-09-01"`.
- `spendLimitAmount.value` (number, optional) — Spend cap amount in the advertiser's currency. Required when `spendLimitType` is `capped`; omit when `uncapped`.
- `spendLimitRenewal` (string, optional) — Renewal cadence: `daily`, `monthly`, `lifetime`, or `undefined`. Required when `spendLimitType` is `capped`.
- `endDate` (date, optional) — Advertiser-local date when the limit expires.

**`scheduledSpendLimit.scheduledSpendLimitUpdates[]`**  
Array of changes to existing scheduled spend limits. Each entry requires `id`, `spendLimitType`, and `startDate`. Optional fields are the same as for creations.

**`scheduledSpendLimit.scheduledSpendLimitDeletions[]`**  
Array of scheduled spend limits to remove. Each entry requires `id` (string) — the identifier of the limit to delete.
...
A new campaign can be created for a specific advertiser by making a POST call to the campaigns endpoint. The request body should specify the type (`Campaign`) and the [campaign attributes](/marketing-solutions/v2026-preview/docs/campaign#campaign-attributes).
...
A full list of error codes can be found on the [Validation Errors](/marketing-solutions/v2026-preview/docs/campaign#validation-errors-and-warnings) page at the end of this section.  
In addition to [general API errors](/marketing-solutions/v2025.07/docs/api-error-types) , you may encounter validation errors when updating a Campaign.
