...
description: Complete reference for Marketing Solutions audience segment types, including FilterBased segments.
---
...
<Info>
  FilterBased audience segments are available across all supported API versions: 2025.01, 2025.04, 2025.07, 2025.10, 2026.01, 2026.07, 2027.01-rc, Preview, and Experimental.
</Info>

FilterBased segments target users based on behavioral signals filtered by seller and product category. They use a boolean algebra tree to combine filters with `and`, `or`, and `not` operators.

| Field | Type | Required | Description |
| ----- | ---- | :------: | ----------- |
| `filterBased` | `FilterBased` | Yes | Filter-based settings. Must be the only segment-type settings object present. |
| `filterBased.algebra` | `AlgebraNode` | Yes | Boolean algebra tree combining filter leaves. |
| `filterBased.algebra.filter` | `FilterDefinition` | No | Leaf node. All filters within a leaf are AND-ed together. |
...
**FilterDefinition** — if multiple filters are set, a signal must match all of them:

| Field | Type | Description |
| ----- | ---- | ----------- |
| `partnerId` | IntegerFilter (int32) | Required when `sellerId` or `productCategory` is set. Operator: `Equals` only. One value. |
| `sellerId` | LongFilter (int64) | Filter for a seller known to the marketplace. Operator: `Equals` only. One value. |
| `productCategory` | ProductCategoryFilter | Filter for one or more product categories. Operators: `Equals` (one) or `In` (one or more). Values: `[{ "id": "string", "level": 1–3 }]`. |
...
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/in-market-interests
```
</EndpointBadge>
...
<EndpointBadge method="get">
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/in-market-brands
```
</EndpointBadge>
...
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/create
```
</EndpointBadge>
    ...
    },
    {
      ...
      "attributes": {
        ...
        "filterBased": {
          "algebra": {
            "filter": {
              "partnerId": { "operator": "Equals", "values": [5855] },
...
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments
```
</EndpointBadge>
...
<EndpointBadge method="post">
```http
https://api.criteo.com/2026-07/marketing-solutions/audiences/search
```
</EndpointBadge>
...
<EndpointBadge method="post">
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/delete
```
</EndpointBadge>
...
<EndpointBadge method="post">
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/compute-sizes
```
</EndpointBadge>
...
<EndpointBadge method="post">
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/estimate-size
```
</EndpointBadge>
...
<EndpointBadge method="patch">
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/{audience-segment-id}/contact-list
```
</EndpointBadge>
...
<EndpointBadge method="get">
```http
https://api.criteo.com/2026-07/marketing-solutions/audience-segments/{audience-segment-id}/contact-list/statistics
```
</EndpointBadge>
...
`filter-based-must-have-one-partner-at-most`

At most one distinct partner ID may be provided across the algebra tree.

`filter-based-fields-must-have-partner-id`

A `partnerId` filter must be present in any leaf that contains `sellerId` or `productCategory`.

`partner-must-be-authorized-for-advertiser`

The partner must be authorized for the owning advertiser.

`sellerid-must-exist`

Every seller ID in the segment must identify an existing seller.

`filter-seller-id-must-have-partner-id`

A `partnerId` must be provided in any leaf that contains a `sellerId`.

`filter-based-product-category-values-must-be-valid`

Every product category value must be non-null and its hierarchy level must be between 1 and 3.

`filter-based-product-category-values-must-be-unique`

Product category values must not be duplicated within the same filter.
