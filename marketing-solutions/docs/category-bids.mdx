---
title: Category Bids
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Performance_Media/v2026_01/docs/Marketing Solutions Endpoints/campaigns/category-bids.md
---
...
<Info>
  **`ID`**
  ...
  The ID is composite of the Ad Set Id, a Criteo category ID and a Category level (1, 2 or 3).   
  In the above example, the id of the category `shoes` is composed of:
...
Category bid is only available on ad set with CPC-based or CPM-based cost controllers. If your ad set uses other cost controller, you must refer to the [Display Multipliers](/marketing-solutions/docs/display-multipliers).
...
This endpoint will allow resetting category bids for all the enabled categories associated with an Ad Set.   
This means that the bid for all categories will be set to the default bid amount of the ad set.
