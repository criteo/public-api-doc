...
## Line Item Core — New Unified Endpoints

A new set of type-agnostic line item endpoints is now available in Experimental. Instead of calling a different endpoint per campaign type, you now use a single contract for all of them.

Pass a `campaignId` and the API infers the line item type from the campaign. Type-specific settings are carried in a details object alongside the shared fields. Initial support covers Onsite Display Auction campaigns using `onsiteDisplayDetails`.
