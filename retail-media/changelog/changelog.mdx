...
**Create a campaign** — `POST /accounts/{accountId}/campaigns`
...
- **[Update a line item](/retail-media/experimental/docs/line-items-core#update-a-line-item-core)** — `PATCH /line-items/{lineItemId}`
...
- **[Bid Strategy Settings](/retail-media/experimental/docs/bidding-strategy-settings)** — `GET /retail-media/line-items/{lineItemId}/bidding-strategy` — Retrieve the current CPM bidding configuration for a line item, including the active strategy (`Standard` or `Adaptive`) and preserved settings for the inactive strategy.

- **[Get Targets](/retail-media/experimental/docs/line-item-targets#get-targets)** — `GET /retail-media/line-items/{lineItemId}/targets` — Retrieve the `ManualKeyword`, `PageType`, and `Category` targets configured for a line item. Supports partial success: if one target type is unavailable, other types are still returned.

- **[Create Targets](/retail-media/experimental/docs/line-item-targets#create-targets)** — `POST /retail-media/line-items/{lineItemId}/targets/create` — Create `ManualKeyword`, `PageType`, and `Category` targets for a line item. A single request can contain targets of more than one type. Supports partial success.

## AI Assistant Bid Multiplier — Now in Stable

The `aiAssistant` bid multiplier field is now available on the `GET` and `PUT /retail-media/line-items/{lineItemId}/bid-multipliers` endpoints, allowing advertisers to adjust bids for AI Assistant placements. This field is available across all stable versions: `2025.01`, `2025.04`, `2025.07`, `2025.10`, `2026.01`, and the current `2026.07`. See the [Bid Multipliers](/retail-media/docs/bid-multipliers) guide for details.
