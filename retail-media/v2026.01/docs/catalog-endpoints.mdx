...
All catalog creation requests must wrap the body parameters in a `data` object with a mandatory `type` field:
...
|-------|----------|-------------|
| `data.type` | Yes | Must be `"RetailMediaCatalogStatus"`. |
| `data.attributes` | Yes | Object containing the request parameters listed below. |
