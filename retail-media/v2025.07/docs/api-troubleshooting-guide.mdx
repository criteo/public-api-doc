...
This guide covers basic checks to identify the source of errors, common issue types, and data latency/discrepancy considerations. If the issue cannot be resolved after these steps, please see our [Escalation Guidelines](https://developers.criteo.com/retail-media/docs/escalation-guidelines).
...
In order to perform a quick check whether an advertiser has given consent to the app, you can query the `[/retail-media/accounts](https://developers.criteo.com/retail-media/docs/accounts-endpoints)` and see if it returns the advertiser ID that you expected.
...
Data latency can also be mistaken for causing issues. Some metrics may have several hours of latency, while others may seem to have longer delays. The following table provides the maximum and median data delays for different metric groups.
...
**Retention** for most of the data, including sales data, is **3 years** unless requested by hourly dimension.
...
The naming pattern provided on [the metrics page](https://developers.criteo.com/retail-media/docs/metrics-and-dimensions) will help you understand how you're comparing the data in different systems. Ask yourself:
...
* [API Error Codes](/retail-media/v2025.07/docs/api-error-codes)
