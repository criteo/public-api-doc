...
```bash cURL
curl -L -X POST "https://api.criteo.com/experimental/retail-media/accounts/625702934721171442/line-items/cap-out-history" \
...
url = "https://api.criteo.com/experimental/retail-media/accounts/625702934721171442/line-items/cap-out-history"
...
$request->setUrl('https://api.criteo.com/experimental/retail-media/accounts/625702934721171442/line-items/cap-out-history');
...
<Warning>
  **Breaking change (2026-07-31):** The `capoutTimes` response keys were renamed from lowercase (`daily`, `hourly`, `monthly`, `total`) to PascalCase (`Daily`, `Hourly`, `Monthly`, `Total`). As per [Criteo's versioning policy](https://developers.criteo.com/criteo-apis/docs/versioning-policy), breaking changes can occur in Experimental. Update any code parsing this response accordingly.
</Warning>
