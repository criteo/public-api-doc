...
The `campaignType` attribute is the discriminator. Type-specific fields are carried in a matching details object alongside the common attributes: `sponsoredProductsDetails` for a `SponsoredProducts` campaign, `onsiteDisplayDetails` for an `OnsiteDisplay` one. Send only the object that matches the type.

<Info>
  The objective and the budget belong to the details object, not to the campaign root. A `SponsoredProducts` campaign sets its own objective; an `OnsiteDisplay` campaign created through this endpoint is always an impressions campaign, so it carries a budget but no objective.
</Info>
...
| `sponsoredProductsDetails` | object | No | `SponsoredProducts` only. Objective and budget of the campaign. Sending it on a campaign of another type is refused. |
| `onsiteDisplayDetails` | object | No | `OnsiteDisplay` only. Budget of the campaign. |
...
A `SponsoredProducts` campaign that runs indefinitely sends exactly `9999-12-30T00:00:00Z` as its `endDate`. Any other far-future date is treated as a real end date. An `OnsiteDisplay` `Auction` campaign cannot run indefinitely and rejects that value.
...
| `budget` | object | No | Financial intent of the campaign. A `Manual` campaign does not accept one. |
...
| Attribute | Type | Required | Description |
| --- | --- | --- | --- |
| `budget` | object | No | Financial intent of the campaign. An amount only: per-period cappings and pacing apply to `SponsoredProducts`. |

### Budget attributes

| Attribute | Type | Required | Description |
| --- | --- | --- | --- |
| `amount` | number | No | Total the campaign may spend over its flight. Must be greater than zero. |
| `cappings` | array | No | `SponsoredProducts` only. Per-period ceilings, at most one entry per period. |
| `cappings[].type` | string | Yes | Period the ceiling applies to. `Daily` or `Monthly`. |
| `cappings[].amount` | number | Yes | Ceiling for the period. |
| `pacing` | object | No | `SponsoredProducts` only. Asks the platform to spread the budget rather than cap it per period. |
    ...
    "data": {
      ...
      "attributes": {
        ...
        "sponsoredProductsDetails": {
          ...
          "budget": {
            "amount": 5000.0,
            "cappings": [{ "type": "Monthly", "amount": 1000.0 }],
...
An `OnsiteDisplay` campaign carries its budget under `onsiteDisplayDetails` and sets no objective:
...
| `400 Bad Request` | `missing-flight-dates` | `scheduleDetails` is absent, or carries only one of the two dates. |
| `400 Bad Request` | `invalid-date-range` | The start date is after the end date. |
| `400 Bad Request` | `invalid-cd-flight-dates` | An `OnsiteDisplay` `Auction` campaign was given the indefinite end date. |
| `400 Bad Request` | `manual-objective-refuses-budget` | A budget was supplied for a `Manual` campaign. |
| `400 Bad Request` | `invalid-budget-amount` | A budget or capping amount is not greater than zero. |
