...
The product pool of a line item holds the products that the line item can promote. Adding a product makes it available to the line item; it does not assign it to a SKU collection.
...
| --- | --- | --- |
| `POST` | `/line-items/{lineItemId}/products/add` | **Coming soon** — Add products to a line item's product pool. |
...
- The request is all-or-nothing. If any `productId` is invalid, the entire request fails and the product pool is left unchanged.
...
- Products are added to the product pool only. They are not assigned to SKU collections.
