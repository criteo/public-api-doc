...
<Info>
  Among the endpoints presented on this page, four are available in [the Stable version.](/retail-media/docs/demand-side-analytics-get-started#/) Only the Accounts level report request is currently in `Preview`. For more details on this endpoint, visit our [dedicated guide](/retail-media/experimental/docs/accounts#/).
</Info>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    ...
    <p>
     Accepted values: refer to <a href="/retail-media/docs/report-types">Report Types</a> page for a complete list of available values
    </p>
    <p>
     Writeable? N / Nullable? N
    </p>
  ...
  <tr>
   ...
   <td>
    ...
    <p>
     Accepted values: refer to <a href="/retail-media/docs/metrics-and-dimensions">Metrics and Dimensions</a> page for a complete list of available values
    </p>
    <p>
     Writeable? N / Nullable? N
    </p>
...
```http
https://api.criteo.com/experimental/retail-media/reports/campaigns
```
</EndpointBadge>
...
```bash cURL
curl -X POST "https://api.criteo.com/experimental/retail-media/reports/campaigns" \
...
url = "https://api.criteo.com/experimental/retail-media/reports/campaigns"
...
$request->setUrl('https://api.criteo.com/experimental/retail-media/reports/campaigns');
...
<EndpointBadge method="post">
```http
https://api.criteo.com/experimental/retail-media/reports/line-items
```
</EndpointBadge>
...
```bash cURL
curl -X POST "https://api.criteo.com/experimental/retail-media/reports/line-items" \
    -H "Authorization: Bearer <MY_ACCESS_TOKEN>" \
    -H "Content-Type: application/json" \
    -d '{
            "data": {
...
url = "https://api.criteo.com/experimental/retail-media/reports/line-items"
...
require_once 'HTTP/Request2.php';
$request = new HTTP_Request2();
$request->setUrl('https://api.criteo.com/experimental/retail-media/reports/line-items');
$request->setMethod(HTTP_Request2::METHOD_POST);
$request->setConfig(array(
  'follow_redirects' => TRUE
));
...
```bash cURL
curl -X POST https://api.criteo.com/experimental/retail-media/reports/accounts \
...
url = "https://api.criteo.com/experimental/retail-media/reports/accounts"
headers = {
...
public class CriteoAccountReport {
    ...
    public static void main(String[] args) {
        try {
            URL url = new URL("https://api.criteo.com/experimental/retail-media/reports/accounts");
...
$apiUrl = 'https://api.criteo.com/experimental/retail-media/reports/accounts'; // Replace {version} accordingly
