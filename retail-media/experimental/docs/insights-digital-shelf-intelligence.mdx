...
| :--- | :------- | :---------- |
| `POST` | `/experimental/retail-media/insights/digital-shelf-intelligence` | Submit a Digital Shelf Intelligence report request |
| `GET` | `/experimental/retail-media/insights/{reportId}/status` | Poll the status of a submitted report |
| `GET` | `/experimental/retail-media/insights/{reportId}/output` | Retrieve the output of a completed report |
...
```http
https://api.criteo.com/experimental/retail-media/insights/digital-shelf-intelligence
```
</EndpointBadge>
...
| `filters` | `object` | No | Optional filter criteria applied to the report. |
| `filters.retailerIds` | `string[]` | No | Retailer IDs to filter on. Must be numeric internal IDs. |
| `filters.brandIds` | `string[]` | No | Brand IDs to filter on. Must be numeric external IDs. |
| `filters.skuIds` | `object[]` | No | SKU filters grouped by retailer. Ignored with a warning when `aggregationLevel` is `brand`. Each entry requires `retailerId` (string) and `retailerSkuIds` (string[]). |
| `filters.categories` | `string[]` | No | Category names to filter on. |
...
```bash cURL
curl --request POST \
  --url 'https://api.criteo.com/experimental/retail-media/insights/digital-shelf-intelligence' \
...
$request->setUrl('https://api.criteo.com/experimental/retail-media/insights/digital-shelf-intelligence');
...
```http
https://api.criteo.com/experimental/retail-media/insights/{reportId}/status
```
</EndpointBadge>
...
```bash cURL
curl --request GET \
  --url 'https://api.criteo.com/experimental/retail-media/insights/<REPORT_ID>/status' \
  --header 'Authorization: Bearer <ACCESS_TOKEN>'
```
...
response = requests.get(
    "https://api.criteo.com/experimental/retail-media/insights/<REPORT_ID>/status",
...
```php PHP
$request = new HTTP_Request2();
$request->setUrl('https://api.criteo.com/experimental/retail-media/insights/<REPORT_ID>/status');
...
<EndpointBadge method="get">
```http
https://api.criteo.com/experimental/retail-media/insights/{reportId}/output
```
</EndpointBadge>
...
```bash cURL
curl --request GET \
  --url 'https://api.criteo.com/experimental/retail-media/insights/<REPORT_ID>/output' \
  --header 'Authorization: Bearer <ACCESS_TOKEN>' \
...
response = requests.get(
    "https://api.criteo.com/experimental/retail-media/insights/<REPORT_ID>/output",
...
```php PHP
$request = new HTTP_Request2();
$request->setUrl('https://api.criteo.com/experimental/retail-media/insights/<REPORT_ID>/output');
$request->setMethod(HTTP_Request2::METHOD_GET);
