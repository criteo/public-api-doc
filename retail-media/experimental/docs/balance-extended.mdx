---
title: Balance (Extended)
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Retail_Media/v2026-preview/docs/Guides for Retail Media API/balances/balance-extended.md
hidden: true
---
...
<Note>
  **A FEW THINGS TO KNOW**
  ...
  Learn more about campaign management with our API [**Here**](/retail-media/experimental/docs/balances)!
</Note>
...
```http
https://api.criteo.com/experimental/retail-media/accounts/{accountId}/balances
```
</EndpointBadge>
...
```bash
curl -L 'https://api.criteo.com/experimental/retail-media/accounts/4/balances' \
...
```json expandable
{
    "metadata": {
        ...
        "currentPageIndex": 0,
        "totalPages": 3,
        "nextPage": "https://api.criteo.com/experimental/retail-media/accounts/4/balances?pageIndex=1&pageSize=25"
    },
    "data": [
        {
            "attributes": {
...
<EndpointBadge method="get">
```http
https://api.criteo.com/experimental/retail-media/accounts/{accountId}/balances/{balanceId}
```
</EndpointBadge>
...
```bash
curl -L 'https://api.criteo.com/experimental/retail-media/accounts/4/balances/3' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>'
```
...
<EndpointBadge method="post">
```http
https://api.criteo.com/experimental/retail-media/accounts/{accountId}/balances/{balanceid}/add-funds
```
</EndpointBadge>
...
```bash
curl -L 'https://api.criteo.com/experimental/retail-media/accounts/4/balances/475388270283501568/add-funds' \
-H 'Content-Type: application/json' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>' \
-d '{
...
<EndpointBadge method="post">
```http
https://api.criteo.com/experimental/retail-media/accounts/{accountId}/balances/{balanceId}/change-dates
```
</EndpointBadge>
...
```bash
curl -L 'https://api.criteo.com/experimental/retail-media/accounts/4/balances/475388270283501568/change-dates' \
-H 'Content-Type: application/json' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>' \
-d '{
...
```bash
curl -L -X PATCH 'https://api.criteo.com/experimental/retail-media/accounts/4/balances/475388270283501568' \
-H 'Content-Type: application/json' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>' \
-d '{
