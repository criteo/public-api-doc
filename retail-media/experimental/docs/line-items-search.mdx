...
| `isPaused` | boolean | Indicates whether the line item is paused. |
...
| `onsiteDisplay.frequencyCapping` | object | Frequency cap for an Onsite Display line item. It is `null` when the line item is uncapped. |
| `onsiteDisplay.frequencyCapping.cappingCount` | integer | Maximum number of times the ads may be shown within the selected duration, from `1` to `20`. |
| `onsiteDisplay.frequencyCapping.cappingDurationType` | string | Period the count applies to: `Session` or `Day`. |
| `onsiteDisplay.auction.isDynamicMatch` | boolean | Onsite Display Auction: whether dynamic matching is enabled. |
| `sponsoredProducts.auction.allInventory` | boolean | Sponsored Products Auction: whether all inventory is targeted. |
| `activationStatus` | string | Current activation status of the line item. Nullable. |
| `fundingStatus` | string | Current funding status of the line item. Nullable. |
