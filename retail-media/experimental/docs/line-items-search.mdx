...
| `includeChildrenAccounts` | No | boolean | _TODO (R&D): confirm whether this includes line items from child or sub-accounts in the account hierarchy._ |
...
| `limit` | No | integer | Maximum number of results to return per page. Defaults to the maximum allowed page size. _TODO (R&D): confirm max value._ |
...
| `lineItemType` | string | Type of line item: `SponsoredProduct` or `CommerceDisplay`. _TODO (R&D): the create/update endpoints return `OnsiteDisplay` — confirm whether `CommerceDisplay` and `OnsiteDisplay` refer to the same type under different names._ |
...
| `isPaused` | boolean | Whether the line item is paused. |
...
_TODO (R&D): confirm full error list — the spec does not enumerate error codes for this endpoint._
