...
| Attribute | Type | Description |
| :-------- | :--- | :---------- |
| `line-item-id` | string | Line item ID. Provided in the URL path. |
| `data` | array | Target resources to create. A request can contain up to 100 targets. |
| `data[].type` | string | Resource type. Set this value to `Target`. |
| `data[].attributes.targetType` | enum | Target type to create: `ManualKeyword`, `PageType`, or `Category`. |
| `data[].attributes.negative` | boolean | Whether the target excludes matching inventory. Applicability depends on the target type. |
| `data[].attributes.bidMultiplier` | number or null | Bid multiplier used when supported for the target and line item. |
| `data[].attributes.manualKeywordTargetDetails` | object | Manual keyword details. Use when `targetType` is `ManualKeyword`. |
| `data[].attributes.pageTypeTargetDetails` | object | Page type details. Use when `targetType` is `PageType`. |
| `data[].attributes.categoryTargetDetails` | object | Category details. Use when `targetType` is `Category`. |
