---
title: Targeting
description: Retrieve and create targets for a line item.
---
...
The Create Targets endpoint creates `ManualKeyword`, `PageType`, and `Category` targets for a line item.
...
| **POST** | `/retail-media/line-items/{line-item-id}/targets/create` | Creates targets for a line item |
...
| `data[].attributes.negative` | boolean | Yes | Whether the target excludes matching inventory. Applicability depends on the target type. |
| `data[].attributes.bidMultiplier` | number or null | No | Bid multiplier used when supported for the target and line item. |
| `data[].attributes.manualKeywordTargetDetails` | object | No | Manual keyword details. Use when `targetType` is `ManualKeyword`. |
| `data[].attributes.pageTypeTargetDetails` | object | No | Page type details. Use when `targetType` is `PageType`. |
| `data[].attributes.categoryTargetDetails` | object | No | Category details. Use when `targetType` is `Category`. |

Each target contains exactly one target details object:

| Target details object | Attribute | Type | Description |
| :-------------------- | :-------- | :--- | :---------- |
| `manualKeywordTargetDetails` | `keywordInput` | string | Keyword text. Maximum 255 characters. |
| `manualKeywordTargetDetails` | `matchType` | enum | Keyword match type: `Broad` or `Exact`. |
| `pageTypeTargetDetails` | `pageType` | enum | Page type to target: `Unknown`, `Search`, `Home`, `Browse`, `Checkout`, `Category`, `ProductDetail`, `Confirmation`, `Merchandising`, `Deals`, `Favorites`, `SearchBar`, `CategoryMenu`, or `AiAssistant`. |
| `categoryTargetDetails` | `categoryId` | string | Category ID to target. |
| `categoryTargetDetails` | `includeChildren` | boolean | Whether the target includes child categories. |
...
A request can contain up to 100 targets and can include more than one supported target type.

Creation supports partial success across target types. If some target types succeed, the endpoint can return `200 OK` with the created targets in `data` and errors for target types that did not succeed. Clients should inspect both `errors` and `warnings` before treating the response as complete.

### Responses

| Response | Meaning | Troubleshooting |
| :------- | :------ | :-------------- |
| 🟢 `200` | The request succeeded completely or partially across target types. | Process the created targets in `data` and inspect `errors` and `warnings`. |
| 🔴 `400` | The request is malformed, contains invalid target data, or contains more than 100 targets. | Correct the request using the response details and retry. |
| 🔴 `401` | The access token is missing, invalid, or expired. | Obtain a valid access token and retry the request. |
| 🔴 `403` | The caller lacks Campaign Manage permission or access to the requested line item. | Verify the bearer token includes the `RetailMedia_Campaign_Manage` scope and the caller has access to the requested line item. |
| 🔴 `500` | An unexpected internal error prevented the endpoint from completing the request. | Retry the request. If the failure persists, contact support. |
| 🔴 `502` | The endpoint could not complete target creation. | Retry the request. If the failure persists, contact support. |
