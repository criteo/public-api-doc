...
<Info>
  The long-term goal is to support line-item core operations across campaign types. Initial documented support targets Onsite Display Auction campaigns and uses `onsiteDisplayDetails`. Its active flight dates are derived from the campaign schedule and returned as `effectiveFlightDates`; do not send `flightDates` when creating a line item. Additional details objects will be documented as support is added.
</Info>
...
The resource `type` is `line-item`. Do not send response-only fields such as `lineItemId`, `type`, `fundingStatus`, `effectiveFlightDates`, or `conquestingSettings` inside `attributes`.
