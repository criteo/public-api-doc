---
title: Overview
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Retail_Media/v2026-preview/docs/Guides for Retail Media API/analytics/demand-side-analytics-overview.md
---
<Warning>
...
|---------|--------------|
| Flexible performance reporting, at any account / campaign / line-item granularity | [`POST /reports/performance`](/retail-media/v2026-preview/docs/performance-report) |
| Budget cap-out and missed-delivery diagnostics (formerly `reportType: capout`) | [`POST /reports/missed-opportunities`](/retail-media/v2026-preview/docs/missed-opportunities-report) |
| Transaction-level attribution log (formerly `reportType: attributedTransactions`) | [`POST /reports/attributed-transactions`](/retail-media/v2026-preview/docs/attributed-transactions-report) |
...
| `reportType: capout` | Use [`POST /reports/missed-opportunities`](/retail-media/v2026-preview/docs/missed-opportunities-report) — note the `capoutMissed*` → `missed*` metric renames |
| `reportType: attributedTransactions` | Use [`POST /reports/attributed-transactions`](/retail-media/v2026-preview/docs/attributed-transactions-report) — note the `adv*` → `advertised*` field renames |

Endpoint-specific rename details live on each endpoint page: see [Missed Opportunities Report](/retail-media/v2026-preview/docs/missed-opportunities-report#migrating-from-reporttype-capout) and [Attributed Transactions Report](/retail-media/v2026-preview/docs/attributed-transactions-report#migrating-from-reporttype-attributedtransactions).
