...
<Warning>
  **This version introduces a major redesign of the Demand Side Analytics reporting API — it is the first version to ship the revamped design.** If you have an integration built against a previous version, it will **not** work unchanged: the endpoints, the request shape, and the `reportType` field have all changed. Read this section and the migration guide below before upgrading.
</Warning>
