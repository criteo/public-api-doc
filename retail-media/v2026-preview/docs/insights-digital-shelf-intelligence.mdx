...
<Info>
  **Regional availability**
  ...
  Currently available for the following countries: **United States (US)**, **Canada (CA)**, and **United Kingdom (UK)**.
</Info>
...
| `startDate` | `string` (date) | Yes | Inclusive start date, as an ISO calendar date `YYYY-MM-DD` (no time component). A value with a time component (e.g. `2025-08-11T00:00:00Z`) is rejected with `400 Bad Request`. If not a Monday, the report is accepted and shifted back to the Monday of that week, with a warning. |
| `endDate` | `string` (date) | Yes | Inclusive end date, as an ISO calendar date `YYYY-MM-DD` (no time component). A value with a time component (e.g. `2025-08-11T00:00:00Z`) is rejected with `400 Bad Request`. If not a Sunday, the report is accepted and shifted forward to the Sunday of that week, with a warning. |
...
| `filters` | `object` | No | Optional filters applied to the report. If omitted, no filters are applied. See the **Filters object** table below. Default: an empty filters object (no filtering). |
...
| ----- | ---- | :------: | ----------- |
| `retailerIds` | `string[]` | No | Retailer IDs to filter on. Must be numeric internal IDs. Default: `[]`. |
| `brandIds` | `string[]` | No | Brand IDs to filter on. Must be numeric external IDs. Default: `[]`. |
| `skuIds` | `object[]` | No | SKU filters grouped by retailer. Ignored with a warning when `aggregationLevel` is `brand`. Each entry requires `retailerId` (string) and `retailerSkuIds` (string[]). Default: `[]`. |
| `categories` | `string[]` | No | Category names to filter on. Default: `[]`. |

<Info>
  **Filters behavior**

  * `filters` is optional. Omit it entirely and the report runs with no filters — identical to omitting all four fields before.
  * Each field inside `filters` keeps its own default (empty array). Supply only the filters you need, e.g. `"filters": { "brandIds": ["2001"] }`.
  ...
  * The `skuIds` + brand-aggregation rule is unchanged: when `aggregationLevel` is `brand`, `filters.skuIds` is ignored and the response carries a warning.
</Info>

<Warning>
  **Breaking change (v2026-preview):** The `retailerIds`, `brandIds`, `skuIds`, and `categories` request parameters have moved into a new optional `filters` object. Update your request bodies to nest these fields under `filters`. Requests that still send them at the top level will have those values ignored (they are no longer read).
</Warning>
...
| `200 OK` + warning | `incompatible-filter` | `filters.skuIds` sent with `aggregationLevel: brand` — filter ignored. | Use `aggregationLevel: sku` when filtering by SKU. |
...
| `400 Bad Request` | `json-serialization-error` | `startDate` or `endDate` includes a time component (e.g. `2025-08-11T00:00:00Z`) instead of a plain `YYYY-MM-DD` date. | Send a calendar date only, with no time component. |
| `400 Bad Request` | `attribute-required` | `filters` sent as `null` instead of an object. | Omit `filters` entirely to apply no filtering; do not send `null`. |
