...
<Warning>
  **Breaking change (2026-07-31):** The `capoutTimes` response keys were renamed from lowercase (`daily`, `hourly`, `monthly`, `total`) to PascalCase (`Daily`, `Hourly`, `Monthly`, `Total`). As per [Criteo's versioning policy](https://developers.criteo.com/criteo-apis/docs/versioning-policy), breaking changes can occur in Preview. Update any code parsing this response accordingly.
</Warning>
