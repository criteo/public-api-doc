...
<Info>
  You can also find the Store Inventory endpoints in our API Reference for [`upsert`](/retail-media/reference/catalog/upsert-store-inventory-per-merchant-id) and [`delete`](/retail-media/reference/catalog/delete-store-inventory-per-merchant-id).
</Info>
