...
<Info>
  **`brandId` — conditionally required**
  ...
  For non-Demand Seller accounts, `brandId` is required when creating a creative. Before calling this endpoint, retrieve the list of valid brand IDs for the account using the [Brands endpoint](/retail-media/docs/brands). Passing an invalid or missing `brandId` on a non-Demand Seller account will result in a validation error.
</Info>
