---
title: Balance (Extended)
original_file: to-migrate/criteo-dev-portal-full-export/merged_export/Retail_Media/v2026-preview/docs/Guides for Retail Media API/balances/balance-extended.md
hidden: true
---
import { EndpointBadge } from "/snippets/EndpointBadge.jsx";

The balance management endpoints are additional sets of balance apps that allow users to have more control of their balances, along with the existing balance endpoints, which help to link existing balances to a campaign. Balance management will provide flexibility to create and update balances. Balance managing also provides additional `POST` and `PATCH` endpoints to provide the ability to edit specific balance attributes, depending on the end-user access level.

***

<Note>
  **A FEW THINGS TO KNOW**

  Learn more about campaign management with our API [**Here**](/retail-media/docs/balances)!
</Note>

## **Endpoints**

<table>
 <thead>
  <tr>
   <th>
    <p>
     Verb
    </p>
   </th>
   <th>
    <p>
     Endpoint
    </p>
   </th>
   <th>
    <p>
     Description
    </p>
   </th>
  </tr>
 </thead>
 <tbody>
  <tr>
   <td>
    <p>
     <b>
      GET
     </b>
    </p>
   </td>
   <td>
    <p>
     <code>
      /accounts/&#123;accountId&#125;/balances
     </code>
    </p>
   </td>
   <td>
    <p>
     Get All Balances
    </p>
   </td>
  </tr>
  <tr>
   <td>
    <p>
     <b>
      GET
     </b>
    </p>
   </td>
   <td>
    <p>
     <code>
      /accounts/&#123;accountId&#125;/balances/&#123;balanceId&#125;
     </code>
    </p>
   </td>
   <td>
    <p>
     Get a specific balance
    </p>
   </td>
  </tr>
  <tr>
   <td>
    <p>
     <b>
      POST
     </b>
    </p>
   </td>
   <td>
    <p>
     <code>
      /accounts/&#123;accountId&#125;/balances
     </code>
    </p>
   </td>
   <td>
    <p>
     Create a new balance
    </p>
   </td>
  </tr>
  <tr>
   <td>
    <p>
     <b>
      POST
     </b>
    </p>
   </td>
   <td>
    <p>
     <code>
      /accounts/&#123;accountId&#125;/balances/&#123;balanceId&#125;/add-funds
     </code>
    </p>
   </td>
   <td>
    <p>
     Add funds to an existing balance
    </p>
   </td>
  </tr>
  <tr>
   <td>
    <p>
     <b>
      POST
     </b>
    </p>
   </td>
   <td>
    <p>
     <code>
      /accounts/&#123;accountId&#125;/balances/&#123;balanceId&#125;/change-dates
     </code>
    </p>
   </td>
   <td>
    <p>
     Change a balance dates
    </p>
   </td>
  </tr>
  <tr>
   <td>
    <p>
     <b>
      PATCH
     </b>
    </p>
   </td>
   <td>
    <p>
     <code>
      /accounts/&#123;accountId&#125;/balances/&#123;balanceId&#125;
     </code>
    </p>
   </td>
   <td>
    <p>
     Update balance fields without changing the funds
    </p>
   </td>
  </tr>
 </tbody>
</table>

   

## **Balance response attributes**

 

## **Get all balances**

This endpoint lists all balances in an account.  Response results will be provided in paginated form.

<EndpointBadge method="get">
```http
https://api.criteo.com/{version}/retail-media/accounts/{accountId}/balances
```
</EndpointBadge>

**Sample Request**

```bash
curl -L 'https://api.criteo.com/{version}/retail-media/accounts/4/balances' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>'
```

**Sample Response** 

```json expandable
{
    "metadata": {
        "totalItemsAcrossAllPages": 54,
        "currentPageSize": 25,
        "currentPageIndex": 0,
        "totalPages": 3,
        "nextPage": "https://api.criteo.com/{version}/retail-media/accounts/4/balances?pageIndex=1&pageSize=25"
    },
    "data": [
        {
            "attributes": {
                "name": "Balance A",
                "poNumber": "",
                "memo": "Spring marketing balance",
                "deposited": 10.00000000,
                "spent": 10.00000000,
                "remaining": 0.00000000,
                "startDate": "2020-04-13",
                "endDate": null,
                "status": "ended",
                "createdAt": "2020-04-13T15:39:48+00:00",
                "updatedAt": "2023-06-13T13:35:53+00:00",
                "balanceType": "capped",
                "salesForceId": "",
                "spendType": "Onsite",
                "id": "3"
            },
            "id": "3",
            "type": "BalanceResponse"
        },
        // ...
        {
            "attributes": {
                "name": "Balance B",
                "poNumber": "",
                "memo": "Winter marketing balance",
                "deposited": 0.01000000,
                "spent": 0.00000000,
                "remaining": 0.01000000,
                "startDate": "2022-06-30",
                "endDate": "2022-07-08",
                "status": "ended",
                "createdAt": "2022-06-30T23:51:15+00:00",
                "updatedAt": "2022-06-30T23:51:15+00:00",
                "balanceType": "capped",
                "salesForceId": "111111",
                "spendType": "Onsite",
                "id": "330495533735960576"
            },
            "id": "330495533735960576",
            "type": "BalanceResponse"
        }
    ],
    "warnings": [],
    "errors": []
}
```

## **Get specific balance**

Retrieves the balance details of one specific balances belonging to an account 

<EndpointBadge method="get">
```http
https://api.criteo.com/{version}/retail-media/accounts/{accountId}/balances/{balanceId}
```
</EndpointBadge>

**Sample Request** 

```bash
curl -L 'https://api.criteo.com/{version}/retail-media/accounts/4/balances/3' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>'
```

**Sample Response** 

```json
{
    "data": {
        "attributes": {
            "name": "Balance A",
            "poNumber": "",
            "memo": "Trade marketing balance for spring 2020",
            "deposited": 10.00000000,
            "spent": 10.00000000,
            "remaining": 0.00000000,
            "startDate": "2020-04-13",
            "endDate": null,
            "status": "ended",
            "createdAt": "2020-04-13T15:39:48+00:00",
            "updatedAt": "2023-06-13T13:35:53+00:00",
            "balanceType": "capped",
            "salesForceId": "",
            "spendType": "Onsite",
            "id": "3"
        },
        "id": "3",
        "type": "BalanceResponse"
    },
    "warnings": [],
    "errors": []
}
```

## **Create a new account balance**

The endpoint allows you to create a new account balance 

<EndpointBadge method="post">
```http
https://api.criteo.com/{version}/retail-media/accounts/{accountId}/balances
```
</EndpointBadge>

**Sample Request** 

```bash
curl -L 'https://api.criteo.com/{version}/retail-media/accounts/4/balances' \
-H 'Content-Type: application/json' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>' \
-d '{
	"data": {
		"attributes": {
			"name": "Balance C - API",
			"deposited": "100",
			"startDate": "2023-08-01",
			"endDate": "2023-08-29",
			"spendType": "onsite",
			"memo": "My API balance",
			"salesForceId": "123456",
			"poNumber": "123"
		},
		"id": "4",
		"type": "RetailMediaBalanceCreation"
	}
}'
```

**Sample Response** 

```json
{
    "data": {
        "attributes": {
            "name": "Balance C - API",
            "poNumber": "123",
            "memo": "My API balance",
            "deposited": 100.00000000,
            "spent": 0.00000000,
            "remaining": 100.00000000,
            "startDate": "2023-08-01",
            "endDate": "2023-08-29",
            "status": "active",
            "createdAt": "2023-08-04T19:43:15+00:00",
            "updatedAt": "2023-08-04T19:43:15+00:00",
            "balanceType": "capped",
            "salesForceId": "123456",
            "spendType": "Onsite",
            "id": "475388270283501568"
        },
        "id": "475388270283501568",
        "type": "BalanceResponse"
    },
    "warnings": [],
    "errors": []
}
```

## **Readjust funds for an existing balance**

Adjust an existing balance funds without changing other balance attributes. 

<EndpointBadge method="post">
```http
https://api.criteo.com/{version}/retail-media/accounts/{accountId}/balances/{balanceid}/add-funds
```
</EndpointBadge>

**Sample Request** 

In the example below, the call requests a \$10 deduction from an existing balance. The original balance was \$100, and after the call, the new balance amount is now \$90. To remove funds from a balance set the `deltaAmount` to a negative number, whereas set a positive amount to add funds. 

```bash
curl -L 'https://api.criteo.com/{version}/retail-media/accounts/4/balances/475388270283501568/add-funds' \
-H 'Content-Type: application/json' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>' \
-d '{
	"data": {
		"attributes": {
			"deltaAmount": "-10",
			"memo": "Balance re-adjustment. Removing $10 from balance",
			"poNumber": "123456"
		},
		"id": "475388270283501568",
		"type": "<string>"
	}
}'
```

**Sample Response** 

```json
{
    "data": {
        "attributes": {
            "name": "Balance C - API",
            "poNumber": "123456",
            "memo": "Balance re-adjustment. Removing $10 from balance",
            "deposited": 90.00000000,
            "spent": 0.00000000,
            "remaining": 90.00000000,
            "startDate": "2023-08-01",
            "endDate": "2023-08-29",
            "status": "active",
            "createdAt": "2023-08-04T19:43:15+00:00",
            "updatedAt": "2023-08-07T15:43:59+00:00",
            "balanceType": "capped",
            "salesForceId": "123456",
            "spendType": "Onsite",
            "id": "475388270283501568"
        },
        "id": "475388270283501568",
        "type": "BalanceResponse"
    },
    "warnings": [],
    "errors": []
}
```

## **Change balance dates**

Update a balance dates without changing all other balance attributes. The endpoint can be used for scenarios with users with limited permissions to make adjustments to a balance. 

<EndpointBadge method="post">
```http
https://api.criteo.com/{version}/retail-media/accounts/{accountId}/balances/{balanceId}/change-dates
```
</EndpointBadge>

**Sample Request**

```bash
curl -L 'https://api.criteo.com/{version}/retail-media/accounts/4/balances/475388270283501568/change-dates' \
-H 'Content-Type: application/json' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>' \
-d '{
	"data": {
		"attributes": {
			"startDate": "2023-08-01",
			"endDate": "2023-08-07",
			"memo": "Updating the balance start and end dates"
		},
		"id": "475388270283501568",
		"type": "RetailMediaBalanceUpdate"
	}
}'
```

**Sample Response**

```json
{
    "data": {
        "attributes": {
            "name": "Balance C - API",
            "poNumber": "123456",
            "memo": "Updating the balance start and end dates",
            "deposited": 90.00000000,
            "spent": 0.00000000,
            "remaining": 90.00000000,
            "startDate": "2023-08-01",
            "endDate": "2023-08-07",
            "status": "active",
            "createdAt": "2023-08-04T19:43:15+00:00",
            "updatedAt": "2023-08-07T16:06:51+00:00",
            "balanceType": "capped",
            "salesForceId": "123456",
            "spendType": "Onsite",
            "id": "475388270283501568"
        },
        "id": "475388270283501568",
        "type": "BalanceResponse"
    },
    "warnings": [],
    "errors": []
}
```

## **Update balance fields without changing the funds**

Update all balance fields without changing the balance funds. The endpoint can be used for scenarios with users with limited permissions to make adjustments to a balance.

<EndpointBadge method="patch">
```http
https://api.criteo.com/{version}/retail-media/accounts/{accountId}/balances/{balanceId}
```
</EndpointBadge>

**Sample Request**

```bash
curl -L -X PATCH 'https://api.criteo.com/{version}/retail-media/accounts/4/balances/475388270283501568' \
-H 'Content-Type: application/json' \
-H 'Accept: application/json' \
-H 'Authorization: Bearer <MY_ACCESS_TOKEN>' \
-d '{
	"data": {
		"attributes": {
			"name": "Balance ABC",
			"poNumber": "65432",
			"salesForceId": "123456",
			"startDate": "2023-08-01",
			"endDate": "2023-08-07",
			"memo": "Updating balance without changing the funds"
		},
		"id": "475388270283501568",
		"type": "RetailMediaBalanceUpdate"
	}
}'
```

**Sample Response**

```json
{
    "data": {
        "attributes": {
            "name": "Balance ABC",
            "poNumber": "65432",
            "memo": "Updating balance without changing the funds",
            "deposited": 90.00000000,
            "spent": 0.00000000,
            "remaining": 90.00000000,
            "startDate": "2023-08-01",
            "endDate": "2023-08-07",
            "status": "active",
            "createdAt": "2023-08-04T19:43:15+00:00",
            "updatedAt": "2023-08-07T16:08:40+00:00",
            "balanceType": "capped",
            "salesForceId": "123456",
            "spendType": "Onsite",
            "id": "475388270283501568"
        },
        "id": "475388270283501568",
        "type": "BalanceResponse"
    },
    "warnings": [],
    "errors": []
}
```
