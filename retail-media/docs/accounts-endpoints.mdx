...
| `type`                | enum          | Account type, with `supply` being the account type for [Retailers](/retail-media/docs/retailers) and `demand` the account type for the different types of advertisers (brand, marketplace sellers, agencies, etc.)Accepted values: `demand`, `supply` Writeable? N / Nullable? N                                             |
| `subtype`             | enum          | Account sub-specific for demand accountsAccepted values: `brand`, `seller` Writeable? N / Nullable? Y                                                                                                                                                                                                           |
...
| `currency`            | string        | Default currency for bulling, budgeting, bid settings & campaign performance metricsAccepted values: 3-chars currency code (in [ISO-4217](https://www.iso.org/iso-4217-currency-codes.html); e.g. `USD`, `EUR`) Writeable? N / Nullable? N                                                                      |
