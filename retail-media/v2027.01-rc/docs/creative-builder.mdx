...
<Note>
  **Getting Started**
  ...
  The creative builder will enable to construction of new creatives through the API. In this document, you will find details regarding the different sections of a creative template variable. A creative comprises five [template variable values](/retail-media/v2027.01-rc/docs/creative-builder#template-variable-value), which is a required array used in the **POST** or **PUT** calls when building a new creative. Any existing [template variable](/retail-media/v2027.01-rc/docs/creative-builder#template-variable) can be found under `sections` of the **GET** Retailer Template endpoint.
</Note>
...
<ParamField path="creativeFormat" type="enum , string" required>
  **Is Nullable?** No
  ...
  `BrandingDisplayGridSolo`,

  `BrandingDisplaySpotlightSolo`,

  `BrandingVideoStandout`,

  `CommerceDisplayGridDuet`,

  `CommerceDisplayGridShelf`,

  `CommerceDisplaySpotlight`,

  `CommerceVideoGridDuet`,
...
<ParamField path="creativeFormatType" type="enum , string">
  **Is Nullable?** No

  Type of creative format. New values added in `2026.07`:
...
Provided when creating ([POST](/retail-media/v2027.01-rc/docs/creative-builder#create-creative)) or updating ([PUT](/retail-media/v2027.01-rc/docs/creative-builder#update-creatives)) a creative.
...
```http
https://api.criteo.com/{version}/retail-media/retailers/{retailerId}/templates
```
</EndpointBadge>
...
* The template variables that can be modified are dependent on each creative template. This information is provided in the [GET | Retailer Templates](/retail-media/v2027.01-rc/docs/creative-builder#get-retailer-templates) response, under the `templateVariables` array. 
...
<Warning>
  **Creative Asset Upload API**
  ...
  The Create Creative endpoint will require an `assetId`. Before generating a new creative ID, upload new creative assets files using the [Creative Asset Upload endpoint](/retail-media/v2027.01-rc/docs/creative-asset-upload) to obtain the `assetId`.
</Warning>

<Info>
  **`brandId` — conditionally required**

  For non-Demand Seller accounts, `brandId` is required when creating a creative. Before calling this endpoint, retrieve the list of valid brand IDs for the account using the [Brands endpoint](/retail-media/v2027.01-rc/docs/brands). Passing an invalid or missing `brandId` on a non-Demand Seller account will result in a validation error.
</Info>
...
<EndpointBadge method="get">
```http
https://api.criteo.com/{version}/retail-media/accounts/{accountId}/creatives/{creativeId}
```
</EndpointBadge>
...
The endpoint will allow for updating existing creatives. The API endpoint request body uses the same `templateVariableValues` model as the one used in the [POST | Create Creative](/retail-media/v2027.01-rc/docs/creative-builder#create-creative) call, but this time allows for updating any of the existing template variable fields. 
...
<Warning>
  **Changes to the Creative ID when Updating Creatives**
  ...
  To ensure that your line item is using the correct `creativeId`, your application will need to call the latest `creativeId` from the [Get Creatives endpoint  ](/retail-media/v2027.01-rc/docs/creative-builder#get-creatives) before associating a new creative to the line item.
</Warning>
