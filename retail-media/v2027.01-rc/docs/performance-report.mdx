...
<Note>
  Enum-style `filters.*` values are case-insensitive on input (e.g. `Online`, `ONLINE`, and `online` are all accepted).
</Note>
...
| `filters.campaignTypes[]` | array of strings | — | Filter by campaign type. Accepted values: `sponsoredProducts`, `onSiteDisplays`. Values constrain eligible campaigns and may affect metric eligibility. |
| `filters.salesChannels[]` | array of strings | — | Filter by sales channel. Accepted values: `online`, `offline`. |
| `filters.mediaTypes[]` | array of strings | — | Filter by media type. Accepted values: `unknown`, `video`, `display`. |
| `filters.buyTypes[]` | array of strings | — | Filter by buy type. Accepted values: `auction`, `preferredDeals`, `sponsorship`. |
| `filters.budgetModels[]` | array of strings | — | Filter by budget model. Accepted values: `CriteoBudget`, `RetailerBudget`. |
| `filters.activationPlatforms[]` | array of strings | — | Filter by activation platform. Accepted values: `CommerceMax`, `PrivateMarket`. |
| `filters.searchTermTypes[]` | array of strings | — | Filter by search term type. Accepted values: `unknown`, `searched`, `entered`. |
| `filters.searchTermTargetings[]` | array of strings | — | Filter by search term targeting strategy. Accepted values: `unknown`, `automatic`, `manual`. |
| `filters.targetedKeywordTypes[]` | array of strings | — | Filter by targeted keyword type. Accepted values: `unknown`, `generic`, `branded`, `conquesting`. |
...
```json
{
  "data": {
    "type": "StatusResponse",
    "id": "c378da57-960e-4ce4-a297-770f9512af56",
    "attributes": {
      "status": "pending",
      "rowCount": 0,
      "fileSizeBytes": 0,
      "md5CheckSum": null,
      "createdAt": "2026-07-28T13:37:17.000Z",
      "expiresAt": null,
      "message": null,
      "id": "c378da57-960e-4ce4-a297-770f9512af56"
    }
...
```json
{
  "data": {
    ...
    "attributes": {
      ...
      "rowCount": 7,
      "fileSizeBytes": 1125,
      "md5CheckSum": "cb448a82ac773d48672cdf30c4fae9de",
      "createdAt": "2026-07-28T13:37:17.000Z",
      "expiresAt": "2026-08-04T13:37:18.000Z",
      "message": "rows_count=7",
      "id": "c378da57-960e-4ce4-a297-770f9512af56"
    }
  }
}
...
With no `format` specified, the output defaults to `json-compact` — a `columns`/`data` array shape rather than an array of row objects:

```json
{
  "columns": ["date", "campaignName", "lineItemName", "impressions", "clicks", "spend", "roas"],
  "data": [
    ["2026-05-01", "Spring Sale - Kitchen Search", "Spring Sale - Kitchen Search - Broad", 2176050, 6461, 15506.65, 22.14],
    ["2026-05-02", "Spring Sale - Kitchen Search", "Spring Sale - Kitchen Search - Broad", 1902829, 6396, 15295.60, 20.36],
    ["2026-05-03", "Spring Sale - Kitchen Search", "Spring Sale - Kitchen Search - Broad", 2061778, 6782, 16234.17, 21.70],
    ["2026-05-04", "Spring Sale - Kitchen Search", "Spring Sale - Kitchen Search - Broad", 1965894, 6287, 15034.17, 24.13],
    ["2026-05-05", "Spring Sale - Kitchen Search", "Spring Sale - Kitchen Search - Broad", 1968091, 6261, 12959.99, 30.44],
    ["2026-05-06", "Spring Sale - Kitchen Search", "Spring Sale - Kitchen Search - Broad", 2075084, 6648, 14762.81, 25.46],
    ["2026-05-07", "Spring Sale - Kitchen Search", "Spring Sale - Kitchen Search - Broad", 2089278, 6551, 15127.54, 20.34]
  ],
  "rows": 7
}
```
...
| `campaignType` | Campaign type: `sponsoredProducts` or `onSiteDisplays`. |
...
| `mediaType` | Media type of the line item: `unknown`, `video`, or `display`. |
| `buyType` | Buy type of the line item: `auction`, `preferredDeals`, or `sponsorship`. |
| `budgetModel` | Budget model of the line item: `CriteoBudget` or `RetailerBudget`. |
| `activationPlatform` | Activation platform of the line item: `CommerceMax` or `PrivateMarket`. |
...
| `searchTermType` | Match type: `Entered`, `Searched`, or `Unknown`. |
| `searchTermTargeting` | Targeting strategy — see note below. |
...
| `targetedKeywordType` | Conquesting strategy: `Conquesting`, `Branded`, or `Generic`. Returns `null` when not applicable. |

<Note>
  **`searchTermTargeting` output values pending confirmation.** Live testing has observed `ManualTargeting`, `AutomaticTargeting`, `Unknown`, and `UnknownTargeting` — two distinct strings for what should be a single "unknown" state. This is being checked with engineering before the exact output value list is finalized here.
</Note>
...
The full legacy → new mapping (all endpoints and every `reportType` value) is documented in the [Analytics Overview — Migrating from the legacy reporting API](/retail-media/v2027.01-rc/docs/demand-side-analytics-overview#migrating-from-the-legacy-reporting-api).
