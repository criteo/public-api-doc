...
| --- | --- | --- |
| Campaign or Line Item ID(s) of the desired report Accepted values: single or list of string/int64 (max 50 ids per call) Writeable? N / Nullable? N |  |  |
| enum | Writeable? N / Nullable? N |  |
| Dimension attributes desired for metrics breakdown for the custom report of the campaign(s) / line item(s). Writeable? N / Nullable? N |  |  |
| Quantitative metrics desired in the custom report of the campaign(s) / line item(s). Writeable? N / Nullable? N |  |  |
| date | Start date to report (inclusive) Writeable? N / Nullable? N |  |
| date | End date to report (inclusive) Writeable? N / Nullable? N |  |
| enum | Writeable? N / Nullable? N |  |
| string | Time zone to consider in the report. Defaults to UTC if omitted. Writeable? N / Nullable? Y |  |
| enum | Writeable? N / Nullable? Y |  |
| enum | Writeable? N / Nullable? Y |  |
| enum | Filter on specific sales channel: online or offline Writeable? N / Nullable? Y |  |
| enum | Format of the report data returned Writeable? N / Nullable? N |  |
...
- [Report Types](/retail-media/v2027.01-rc/docs/report-types)
- [Metrics and Dimensions](/retail-media/v2027.01-rc/docs/metrics-and-dimensions)