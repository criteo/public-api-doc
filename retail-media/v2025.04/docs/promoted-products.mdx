...
| `lineItemId`\* | string    | [Line Item](doc:line-items) ID, in which the product is to be promoted; required in the endpoints' path to define in which line item to perform the actionAccepted values: string of int64 Writeable? N / Nullable? N                                                                                                                                  |
| `bidOverride`  | decimal   | Bid value for the specific product; overrides `targetBid` specified on the [Line Item](doc:line-items) and must satisfy `minBid` from [Catalog](doc:catalogs) (input excludes platform fees).Accepted values: `bidOverride` ≥ `minBid` Writeable? N / Nullable? Y                                                                                      |
