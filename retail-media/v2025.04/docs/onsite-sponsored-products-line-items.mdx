...
| `campaignId`\*        | string    | [Campaign](doc:campaign) IDAcceptable values: int64 Writeable? N / Nullable? N                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `monthlyPacing`       | decimal   | Amount the line item can spend per calendar month, in the [Account](doc:account) timezone (optional)Accepted values: `null` or greater than zero Default: `null` Writeable? Y / Nullable? Y                                                                                                                                                                                                                                                                                                                                                                          |
| `dailyPacing`         | decimal   | Amount the line item can spend per calendar month, in the [Account](doc:account) timezone (optional) It resets each day overwritten by calculation if `isAutoDailyPacing` is enabled; uncapped if omitted of set to `null`Accepted values: `null` or greater than zero Default: `null` Writeable? Y / Nullable? Y                                                                                                                                                                                                                                                    |
...
| `status`              | enum      | Line item status, can only be updated by user to `active` or `paused` Other values are applied automatically depending start/end dates, financials or missing attributes required for line item to deliver ads. To understand the conditions that cause status changes, check out [Campaign & Line Item Status](doc:campaign-lineitem-status)Accepted values: `active`, `paused`, `scheduled`, `ended`, `budgetHit`, `noFunds`, `draft`, `arquived` Writeable? N / Nullable? N                                                                                       |
