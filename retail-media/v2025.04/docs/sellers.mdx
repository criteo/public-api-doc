...
| ---------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `accountId`\*    | string    | [Account](/retail-media/v2025.04/docs/account) ID, to consider in the search for Seller accountsAccepted values: int64Writeable? N / Nullable? N                                                                             |
...
<CodeGroup>
  ```bash cURL
  curl -L -X POST 'https://api.criteo.com/{version}/retail-media/account-management/accounts/sellers/search' \
...
| 🔴 `403` | Authorization Error | `Resource access forbidden: does not have permissions` | API user does not have the authorization to make requests to the account ID. For an authorization request, follow the [authorization request](/retail-media/v2025.04/docs/authorization-requests) steps |
