...
2. [Account](/retail-media/v2025.01/docs/account)
...
| `type`                | enum          | Account type, with `suply` being the account type for [Retailers](doc:retailers) and `demand` the account type for the different types of advertisers (brand, marketplace sellers, agencies, etc)Accepted values: `demand`, `supply` Default: `json` Writeable? N / Nullable? N                                 |
