...
| `bidOverride`  | decimal   | Bid value for the specific product; overrides `targetBid` specified on the [Line Item](doc:line-items) and must satisfy `minBid` from [Catalog](doc:catalogs) (input excludes platform fees)Accepted values: `bidOverride` ≥ 0.0 Default: `0.0` Writeable? Y / Nullable? Y                                                                                        |
| `status`       | enum      | Status of Promoted Product; can only be updated to active or paused using the pause/un-pause endpoints below. For more details about each state, check out [Campaign, Line Item & Products Status](doc:campaign-lineitem-status)Accepted values: `active`, `paused`, `scheduled`, `ended`, `budgetHit`, `noFunds`, `draft`, `archived` Writeable? Y / Nullable? N |
...
| ----------- | --------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `offsite`   | integer   | Pagination parameter, see [API Response](doc:api-response)                                                                        |
| `limit`     | integer   | Pagination parameter, see [API Response](doc:api-response)                                                                        |
