...
Review [Steps 2 & 3](/retailer-integration/docs/video-player-implementation-ios-app#/step-2-build--download-ios) from the *Instruction* section above. Ensure the framework is added to the target, set to **Embed & Sign**, and the import matches the namespace of your organization. (`#if canImport(OMSDK_)`).
...
* Review [Step 4](/retailer-integration/docs/video-player-implementation-ios-app#/step-4-add-the-omid-js-library) from the *Instruction* section above - ensure `omsdk-v1.js` is bundled (Target Membership enabled).

* Review [Step 6](/retailer-integration/docs/video-player-implementation-ios-app#/step-6-activate-om-sdk) from the *Instruction* section above - Call `OMIDSDK.shared.activate()` once at app launch.
