...
* The video player used by the client **must be able to read VAST XML files** (e.g., Google Interactive Media Ads player or a native HTML player with an additional JavaScript layer that can handle VAST XML files). Please visit the [Video Player Implementation page](/retail-media-delivery/docs/video-player-implementation-guide-desktopmobile) for more details.
