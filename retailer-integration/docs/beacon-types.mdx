...
Painted slot indices represent the position of each product as rendered in the UI within a placement. Providing slot indices enables accurate measurement and attribution at the slot level.
...
Slot indices must reflect the actual visual position of each product in the placement (e.g., left-to-right, top-to-bottom).
