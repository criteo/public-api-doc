...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>Key-value pairs to send as the <code>application/x-www-form-urlencoded</code> POST body via <code>navigator.sendBeacon(url, data)</code>. Common keys include <code>rm_e</code> (encrypted beacon string), <code>sku</code> (rendered SKU IDs), <code>slot</code> (slot indices), <code>ex</code> (unrendered SKU IDs), and <code>st</code> (exclusion reasons).</p></td>
  </tr>
 </tbody>
</table>
