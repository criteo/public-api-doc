...
Allows you to exclude a specific product from one or more Criteo activation channels, even if the product is otherwise eligible for delivery.

**Specifications**

* Accepted values are: `PERF_MEDIA`, `RM_ONSITE`, and `RM_OFFSITE`.
* Multiple values can be combined, separated by a comma.
* Leave empty, or omit the field, if the product should not be excluded from any channel.
* Any value other than the three listed above is not recognized and will exclude the product from all channels as a fail-safe.
* Limit: up to 3 values
* Type: `String`
* Example: `PERF_MEDIA,RM_OFFSITE`
